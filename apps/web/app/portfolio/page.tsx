							...
							<TabsList className="bg-multi-cinza-claro h-auto p-1 flex-wrap justify-center">
								{categories.map((cat) => (
									<TabsTrigger
										key={cat}
										value={cat}
										className="data-[state=active]:bg-multi-roxo data-[state=active]:text-white transition-all px-6 py-2 rounded-md text-base"
									>
										{cat}
									</TabsTrigger>
								))}
							...
							<TabsContent key={cat} value={cat} className="animate-fade-in-up stagger-1 mt-0">
								<div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
									{PROJECTS.filter((p) => cat === "Todos" || p.category === cat).map((project) => (
										<Card
											key={project.slug}
											className="overflow-hidden border-none shadow-card hover:shadow-card-hover transition-all duration-smooth group bg-white h-full flex flex-col"
										>
