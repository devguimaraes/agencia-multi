...
export function Footer() {
	return (
		<footer className="bg-multi-black text-white">
			<div className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
				<div className="grid grid-cols-1 gap-12 md:grid-cols-3">
					{/* Col 1: Logo + tagline */}
					<div className="flex flex-col gap-4">
						<Link href="/" aria-label="Multi BR — Página inicial">
							<Image
								src="/brand/logomarca-branca.png"
								alt="Multi BR"
								width={160}
								height={50}
						...
						<p className="max-w-xs text-gray-400 text-sm leading-relaxed">
							Estratégia digital sem enrolação. Branding, conteúdo e tráfego para quem quer
							resultado.
						</p>
						{/* Redes sociais */}
						<div className="flex items-center gap-3 pt-2">
							{SOCIAL_LINKS.map(({ href, label, icon: Icon }) => (
								<a
									...
									aria-label={label}
									className="flex h-9 w-9 items-center justify-center rounded-full border border-white/10 text-gray-400 transition-all hover:border-multi-amarelo hover:text-multi-amarelo"
								>
									<Icon />
								</a>
							))}
					...
					{/* Col 2: Links rápidos */}
					<div>
						<h3 className="mb-5 font-semibold text-sm text-white uppercase tracking-widest">
							Links rápidos
						</h3>
							...
							<ul className="flex flex-col gap-3">
								{QUICK_LINKS.map(({ href, label }) => (
									<li key={href}>
										<Link
											href={href}
											className="text-gray-400 text-sm transition-colors hover:text-multi-amarelo"
										>
											{label}
										</Link>
									</li>
						...
						<ul className="flex flex-col gap-3 text-sm text-gray-400">
							<li>
								<a
									...
									target="_blank"
									rel="noopener noreferrer"
									className="flex items-center gap-2 transition-colors hover:text-multi-amarelo"
								>
									<PhoneIcon />
									(21) 96971-5247
								</a>
				...
				{/* Barra inferior */}
				<div className="mt-12 flex flex-col items-center justify-between gap-4 border-white/10 border-t pt-8 text-gray-600 text-xs md:flex-row">
