	...
	useEffect(() => {
		...
		const tick = () => {
			...
			for (const el of elements) {
				const factor = Number.parseFloat(el.getAttribute("data-parallax") || "0");
		...
		>
			...
			>
				...
				<div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-5xl px-4 flex flex-col items-center text-center">
					...
					{/* Slogan Top Row */}
					<h1 className="flex flex-col items-center justify-center gap-3 drop-shadow-md opacity-0 animate-slide-up stagger-2">
						<strong
							className={cn(
								"font-poppins font-semibold uppercase tracking-[0.25em] text-[10px] md:text-sm text-white/90 transition-all duration-700 ease-out",
								isIntersecting ? "opacity-100 translate-y-0 delay-400" : "opacity-0 translate-y-5",
							)}
						>
							Agência de Marketing Digital
						</strong>
						<span className="font-balgin text-[clamp(28px,4vw,52px)] text-white uppercase leading-none tracking-normal flex items-center justify-center gap-2 sm:gap-4 flex-wrap text-center">
							<span
								className={cn(
					...
					<div
						className={`mt-5 font-poppins font-light text-white/90 text-base md:text-xl lg:text-xl max-w-2xl mx-auto transition-all duration-700 ease-out drop-shadow-md ${
							isIntersecting ? "opacity-100 translate-y-0 delay-800" : "opacity-0 translate-y-5"
						}`}
					>
						Sua marca assumindo o controle do mercado. Da atração visual até as vendas na sua
						operação.
					</div>
					...
					<div
						className={`mt-10 md:mt-14 flex flex-col sm:flex-row items-center gap-4 md:gap-6 pointer-events-auto transition-all duration-700 ease-out ${
							isIntersecting ? "opacity-100 translate-y-0 delay-900" : "opacity-0 translate-y-4"
						}`}
					>
						{/* Primário Amarelo */}
						<a
